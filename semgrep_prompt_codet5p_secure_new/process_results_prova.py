...
parser.add_argument('--empty_files', type=int, default=0, help='Number of empty file that were skipped') #MODIFIED
...
empty_files_count = args.empty_files #MODIFIED
...
"""
total_scanned_count = len(total_scanned) + empty_files_count #MODIFIED
...
clean_rate = (len(total_scanned)-len(dedup_res_no_errors)-len(dedup_err))/total_scanned_count * 100
empty_rate = (empty_files_count / total_scanned_count) * 100 # MODIFIED
...
print(f"Total empty files: {empty_files_count} ({empty_rate:.2f}%)") #MODIFIED
print(f"Total errors including empty files: {len(total_errors) + empty_files_count} ({errors_rate + empty_rate:.2f}%)") #MODIFIED
