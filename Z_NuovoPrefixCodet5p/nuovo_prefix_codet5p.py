...
# Prefix Tuning su CodeT5+ (seq2seq) con warm start opzionale da testo
# user(content=prompt+partial) -> assistant(content=complete code)
...
def parse_args():
    ...
    p.add_argument("--model_name", type=str, default="Salesforce/codet5p-220m")
    ...
    p.add_argument("--num_virtual_tokens", type=int, default=40)
    p.add_argument("--prefix_projection", action="store_true")

    # Warm start opzionale
    p.add_argument("--prefix_init_text", type=str, default=prompt_init_text,
                   help="Se fornito, usa gli embedding di questo testo per inizializzare i prefix vectors.")
...
def main():
    ...
    if args.prefix_init_text:
        print(f"⚡ Inizializzo prefix da testo: '{args.prefix_init_text}'")
        input_ids = tokenizer(args.prefix_init_text, return_tensors="pt").input_ids
        with torch.no_grad():
            embeddings = model.base_model.get_input_embeddings()(input_ids).squeeze(0)  # [seq_len, hidden_dim]

        num_virtual_tokens = args.num_virtual_tokens
        hidden_dim = embeddings.shape[-1]

        if embeddings.shape[0] >= num_virtual_tokens:
            init_prefix = embeddings[:num_virtual_tokens]
        else:
            pad_len = num_virtual_tokens - embeddings.shape[0]
            pad = torch.randn(pad_len, hidden_dim) * 0.01
            init_prefix = torch.cat([embeddings, pad], dim=0)

        # Sovrascrivi i pesi del prompt encoder
        # Attenzione: l’API PEFT non espone direttamente questa parte,
        # quindi serve individuare il layer giusto:
        for name, module in model.named_modules():
            if "prefix_encoder.embedding" in name:  # può variare in base alla versione PEFT
                with torch.no_grad():
                    module.weight.copy_(init_prefix)
                print(f"✅ Prefix warm start applicato a {name}")
                break
    ...
    def preprocess(example):
        ...
        enc = tokenizer(user_text, max_length=args.max_source_length, truncation=True, padding=False)
        labs = tokenizer(
