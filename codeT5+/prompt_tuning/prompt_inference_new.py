...
def load_model_and_tokenizer(model_or_adapter_path: str, dtype, device: torch.device):
    ...
    if is_adapter_dir:
        if not _HAS_PEFT:
            raise RuntimeError("PEFT non disponibile. Installa 'peft' per usare gli adapter")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inference CodeT5+ prompt-tuned model")
