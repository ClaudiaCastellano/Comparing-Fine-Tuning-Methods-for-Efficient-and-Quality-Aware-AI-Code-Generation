...
# AdapterHub / adapter-transformers
try:
...
def parse_args():
    ...
    p.add_argument("--train_file", type=str, required=True, help="Path a JSON/JSONL di training")
    ...
    p.add_argument("--output_dir", type=str, default="./codet5p-out-adapter")
    ...
    # ---- Adapter config ----
    p.add_argument("--adapter_name", type=str, default="code_adapter")
    p.add_argument("--adapter_arch", type=str, default="houlsby", choices=["pfeiffer","houlsby"])
    p.add_argument("--adapter_reduction_factor", type=int, default=16, help="bottleneck size")
...
def main():
    ...
    if not _HAS_ADAPTERS:
        raise RuntimeError("Manca adapter-transformers. Installa con: pip install adapter-transformers")
    set_seed(args.seed)
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        evaluation_strategy=args.eval_strategy if "validation" in dsd_proc else "no",
    ...
    adapter_size_mb = get_dir_size_mb(args.output_dir)
    log_info = {
        "technique": "adapter_tuning_seq2seq",
    ...
    print("✅ Adapter training concluso. Report salvato in resource_report.json")
